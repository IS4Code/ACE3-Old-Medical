...
private _fnc_selectItem = {
    ...
    switch (_currentItemsIndex) do {
        ...
        case 19: {
            if (_item == "") then {
                if (cba_disposable_replaceDisposableLauncher && {!isNil "cba_disposable_LoadedLaunchers"} &&
                {
                    if (cba_disposable_LoadedLaunchers isEqualType createHashMap) then { // after CBA 3.18
                        ...
                        !isNil {cba_disposable_LoadedLaunchers getVariable secondaryWeapon GVAR(center)}
                    }
                }) exitWith { TRACE_1("ignoring unload of disposable",secondaryWeapon GVAR(center)); };
